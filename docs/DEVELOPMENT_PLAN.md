...
> **Status: done** on branch `phase-8-release`, without the documentation
> site (dropped).
> - README rewritten as a quickstart and guide; `docs/coming-from-r.md` maps
>   the R package's functions to Python; `CHANGELOG.md` and an MIT `LICENSE`.
> - Complete PyPI metadata (PEP 639 license, classifiers, URLs, the
>   `pandas` extra); the name `activipyinfo` was free on PyPI and TestPyPI.
> - `.github/workflows/release.yml`: a `vX.Y.Z` tag checks the version,
>   runs the checks and tests, publishes to PyPI with trusted publishing and
>   creates the GitHub release; a manual run publishes to TestPyPI.
> - Phase 7 is skipped for now.
